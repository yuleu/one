...
function TBurl(li){
return win.$(li).data('data').link.replace(/\/data\/music\/file\?link=/,'');
}
var BTit=" title='[点击直接下载] - By Yulei'";
//登录检查，修复无高品质问题
if (DM('#bai')){Lc.href=TBurl(Bl[0])};if(Bld){Bld.style="color:yellow;background-color:gold;font-size:20px;font-weight:bold";}
if((Bld != 'javascript:;') || (win.isLogin=='true')){
win.isCloudVip=true;//collectBtn
win.$(".extra-ad-link, .vip-power").remove();
if (Bhr){
Bhr.innerHTML="<a href='javascript:;' id='SCo' "+BTit+">"+Bhr.innerHTML+"</a>";
 win.collectBtnToDone=function(){};
win.$("#SCo").click(function(){
win.$('.song-collect').removeClass("song-collect-not-vip").click();
setTimeout(function(){Lc.href="http://yinyueyun.baidu.com/data/cloud/downloadsongfile?songIds="+win.$('.song-collect').data('btndata').ids+"&rate=320";}, 555);
});}
if (Bl[1]){Bl[1].innerHTML="<a href='"+TBurl(Bl[1])+"'"+BTit+">"+Bl[1].innerHTML+"</a>";
}}if (Bl[0]){Bl[0].innerHTML="<a href='"+TBurl(Bl[0])+"'"+BTit+">"+Bl[0].innerHTML+"</a>";
}
...
if(Bld){Bld.style.color="yellow";Bld.style.backgroundColor="gold";Bld.style.fontSize="20px";}
}
}
//以下截获Baidu下载事件
if (/\/\d+$/.test(lurl)){
...
var Turl=(typeof win.url=="string");var ii=Turl?((typeof win.server=="string") ? "23":"39"):"33";var f39=_Q('table').getElementsByTagName('font')[ii]; var MusU=Turl ? win.url : win.flashvars.f.replace(/Serverurl/,"");
f39.outerHTML+=' <a href="http://s5.hcdj.com:8080/uuauth/'+MusU+'" '+Tit+Sty0+'>下载 </a>';
}

//免登录，免Y币 //By Yulei 2013.03.09
function Djye(){ //DJ耶耶网
...
// 感谢Sep提供接口，仅供学习研究-24小时内请删除或停止使用。By Yulei 2013.05.07
function YYTi(){ //音悦台，免登录
var Yeio='此MV不提供这种品质哦！\r 请选择其它的下载！~';
if (DM('/playlist/')) {
_Q('#nowPlaying_artist').outerHTML+='<a class="play_operate_down" '+Tit+' name="0" id="odown" style="z-index:999;display:none;line-height:24px;text-indent:0px;position:absolute;top:629px;left:120px;height:24px;color:green"><span style="background-position:-37px -217px;margin:5px 4px;" class="ico_operate_down03"></span><span id="p1">标清</span> <span id="p2">高清</span><span id="pdon" class="play_operate_down" style="position:absolute;padding:0px 10px;width:25px;display:none;color:red">超清</span></a><style>#odown span:nth-child(n+2){cursor:pointer;} #odown:hover #pdon{display:inline-block !important;}</style>';
win.$$('#mvlist>li').each(function(el,i){
el.name=i;
el.onmouseover=function(e){var Od=_Q('#odown'); Od.name=this.name;Od.style.display="block";Od.style.left=e.clientX-30+"px";
} });
win.$$('#odown span[id]').each(function(e){
e.addEventListener('click',function(){
var ii=this.id == "p1" ? "0" : this.id == "p2" ? "1" : "2",Ni=this.parentNode.name;
...
_Q('#yinyuetaiplayer').outerHTML+='<a class="play_operate_down" '+Tit+' id="odown" style="position:absolute;z-index:9999;top:-27px;right:10px;color:green"><span style="background-position:-37px -217px;margin:5px 4px;" class="ico_operate_down03"></span><span id="p1">标清</span> <span id="p2">高清</span><span id="pdon" class="play_operate_down" style="position:absolute;padding:0px 10px;width:25px;display:none;color:red">超清</span></a><style>#odown span:nth-child(n+2){cursor:pointer;} #odown:hover #pdon{display:inline-block !important;}</style>';
_Q('#pdon').onclick=_Q('#p1').onclick=_Q('#p2').onclick=function DownS(){
var ii=this.id == "p1" ? "0" : this.id == "p2" ? "1" : "2";
new win.Request.JSONP({url:"/main/get-video-info",method:"GET",cache:true,data:"json=true&videoId="+win.Y.player.object.objectId,onSuccess:function(data){
var viNum=data.videoInfo.coreVideoInfo.videoUrlModels;
if (!viNum[ii]){win.infoMessage(Yeio,2880);return}
Lc.href=viNum[ii].videoUrl;
}}).send();
  }
 }
}
